(ns ncic.main
  "Kotodama WASM entrypoint for the Ncic clean-room actor (L4) — Clojure port.
  ...
  facts (`ncic.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"ncic.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ncic-compat" :tier tier :entities entities} 200])
